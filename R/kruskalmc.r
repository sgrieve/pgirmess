...

{
    ...
    names(dif)<-vname
choice<-pmatch(cont,c("one-tailed","two-tailed"),nomatch=3)
...
if (choice==3) stop("Values must be 'one-tailed' or 'two-tailed', partial matching accepted")
    
}

output<-list(statistic=stat,p.value=probs, dif.com=data.frame(obs.dif=dif,critical.dif=lims,difference=ifelse((dif-lims)>0,TRUE,FALSE)))
